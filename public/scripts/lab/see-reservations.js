...
$(document).ready(function () {
    ...
    async function addRow(reservationId, building, room, date, startTime, endTime, reservedBy, resDate, resTime) {
        ...
        const [editable, deletable, happeningNow, cancelled] = await Promise.all([
            fetch(`/reservations/${reservationId}/checkEditable`).then(res => res.json()),
            fetch(`/reservations/${reservationId}/checkDeletable`).then(res => res.json()),
        ...
        if (cancelled) {
            tr.innerHTML = `<td class="py-3 text-slate-800 pl-[19px] text-sm text-center">${building}</td>
                        ...
                        <td class="border-b border-default px-4 py-4 text-center">
                            <span class="text-sm text-red-500 font-medium leading-none">Cancelled</span>
                        </td>
                        <td class="py-3 pl-4">
